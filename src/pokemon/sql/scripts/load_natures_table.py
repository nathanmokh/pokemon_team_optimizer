...
def load_natures_table():
    ...
    def get_nature(nature_id: int) -> dict:
        ...
        if response.status_code != 200:
            print(f"Failed to retrieve nature with ID {nature_id}. HTTP Status Code: {response.status_code}")
            return None
    ...
    currently_loaded_nature_ids = {record[0]: None for record in currently_loaded_nature_ids}
