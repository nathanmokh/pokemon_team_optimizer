...
#Get Half Damage Functions
    ...
    def get_type_data(type_id: int) -> dict:
        ...
        if pokemon_data.status_code != 200:
            print(f"Failed to retrieve type with ID {type_id}. HTTP Status Code: {pokemon_data.status_code}")
            return None
        ...
        type_relations = get_damage_relations(pokemon_data)
        double_damage_from = get_double_damage_from(type_relations)
        double_damage_to = get_double_damage_to(type_relations)
        half_damage_from = get_half_damage_from(type_relations)
        half_damage_to = get_half_damage_to(type_relations)
    ...
    currently_loaded_type_ids = {record[0]: None for record in currently_loaded_type_ids}
